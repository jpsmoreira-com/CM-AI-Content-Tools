...
- `tfs-doc-automation-mvp`: shared Content AI assets are now installed with APM, following the `CM-AI-Content-Skills` 1.0 contract. `scripts/sync-content-ai-assets.sh` runs `apm install`/`apm compile` (honoring a committed portal `apm.yml`, or generating a git-excluded one pointing at the local checkout), cleans up the previous `.agents/content-ai` + dotagents layout, and the pipeline's instruction discovery reads `.github/instructions/` and `.claude/rules/`. `CONTENT_AI_SYNC_ROOT_AGENTS` and `CONTENT_AI_DOTAGENTS*` are gone; `CONTENT_AI_APM_VERSION`, `CONTENT_AI_APM_DEPENDENCY` and `CONTENT_AI_APM_INSTALL_CLI` replace them.
