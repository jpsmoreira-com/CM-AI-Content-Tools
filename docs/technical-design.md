...
The recommended devcontainer route for GitHub Copilot is `copilot_cli`. It executes GitHub Copilot CLI in non-interactive mode inside the work-item branch, passes the dashboard-selected model with `--model`, supplies the complete handoff through `-p`, and uses `--no-ask-user` for unattended execution. The command grants read, write, and shell capabilities needed to inspect, edit, and validate documentation, while denying Git commit, push, reset, clean, and removal commands. The standard `agent-result.json` remains the handoff boundary: only the dashboard validates the green light and performs commit, push, final report publication, and Draft PR creation.

`copilot_cli` authenticates through GitHub Copilot CLI. For interactive devcontainer setup, Settings starts the OAuth device flow when preflight detects missing authentication. The configurable `GitHub Copilot Host` defaults to `https://github.com` and must be set to the organization's GitHub Enterprise Cloud data-residency host when applicable. The CLI configuration home defaults to a persisted `CONTENT_AI_SETTINGS_PATH/copilot-cli` directory when that environment variable is available, so a completed authorization survives DevContainer recreation. Non-interactive service deployments should supply a supported fine-grained GitHub token through `COPILOT_GITHUB_TOKEN` instead. The organization must enable the GitHub Copilot CLI policy for entitled users.
