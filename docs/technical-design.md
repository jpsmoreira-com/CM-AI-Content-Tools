...
CLI runtime checks are evaluated inside the configured execution runtime, not on the WSL host by assumption. For devcontainers, the bootstrap sets `NPM_CONFIG_PREFIX`, installs Codex CLI and GitHub Copilot CLI into the container user's npm prefix when enabled, and the dashboard preflight checks the selected provider there. Codex uses `CODEX_HOME`; GitHub Copilot CLI keeps its native `~/.copilot` state path, which bootstrap links to the persisted Content AI settings location. The executable and its authentication must both be available inside the container runtime.
