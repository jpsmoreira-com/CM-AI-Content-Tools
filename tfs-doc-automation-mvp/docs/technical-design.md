...
The recommended devcontainer route for GitHub Copilot is `copilot_cli`. It executes GitHub Copilot CLI in non-interactive autopilot mode inside the work-item branch, passes the dashboard-selected model with `--model`, supplies the complete handoff through `-p`, and uses `--no-ask-user` for unattended execution. The command grants read, write, and shell capabilities needed to inspect, edit, and validate documentation, while denying Git commit, push, reset, clean, and removal commands. If an otherwise successful CLI run omits `agent-result.json`, the wrapper issues a separate reporting-only request and stores its JSON response; an unparseable response becomes a conservative manual-review result instead of leaving the item waiting forever. After a service restart, persisted flows are resumed from their saved branch and result state rather than recreated. The standard `agent-result.json` remains the handoff boundary: only the dashboard validates the green light and performs commit, push, final report publication, and Draft PR creation.

`copilot_cli` authenticates through GitHub Copilot CLI. For interactive devcontainer setup, Settings starts the OAuth device flow when preflight detects missing authentication. The configurable `GitHub Copilot Host` defaults to `https://github.com` and must be set to the organization's GitHub Enterprise Cloud data-residency host when applicable. The CLI uses its native `~/.copilot` state directory; bootstrap migrates that directory to `CONTENT_AI_SETTINGS_PATH/copilot-home` and creates a symlink back to the native location, so a completed authorization survives DevContainer recreation without relying on an unsupported alternate home. The preflight explicitly detects the CLI's no-authentication response even when the executable exits successfully. In headless Linux runtimes where no system keychain is available, the setup accepts the Copilot CLI plaintext-storage fallback and restricts the persisted state directory to the DevContainer user. Non-interactive service deployments should instead supply a supported fine-grained GitHub token through `COPILOT_GITHUB_TOKEN`. The organization must enable the GitHub Copilot CLI policy for entitled users.

`vscode_bridge` remains available as a compatibility provider. It is a private workspace extension (`criticalmanufacturing.cmf-content-ai-pipeline-bridge`) that uses the public VS Code Language Model API instead of attempting to drive the Chat UI. The dashboard writes `bridge-job.json` beside the work item package; the extension reads the package from the active workspace, performs a bounded read/search/apply loop, and writes the existing `agent-result.json` contract.
...
When `CONTENT_AI_SETTINGS_PATH` is configured, the dashboard mirrors `.env`, `config/tfs_dashboard.local.json`, and the local Git credential store mirror into that persistent folder and restores them when a fresh central checkout is created. Target devcontainers should bind-mount the WSL host folder that contains the repositories (conventionally `/workspaces`, but any path works) into the container, keep the central tool checkout at `<repos-parent>/CM-AI-Content-Tools`, keep persistent settings at `<repos-parent>/.content-ai-settings/tfs-doc-automation-mvp`, and keep the active target repository mounted separately at `/app`. Mounting the full repositories tree also keeps Git worktree metadata visible when `/app/.git` points to a shared Git directory outside the opened worktree.
...
`scripts/sync-content-ai-assets.sh` runs `apm install` and `apm compile` in the target workspace. A portal that commits `apm.yml` and `apm.lock.yaml` gets exactly its pinned release (`--frozen`); a portal that has not adopted APM yet gets a generated, git-excluded manifest pointing at `CONTENT_AI_APM_DEPENDENCY` (default: the published `main` branch until a release tag exists; a local checkout path works offline). APM deploys skills to `.agents/skills/` and `.claude/skills/`, subagents to `.github/agents/`, `.claude/agents/` and `.codex/agents/`, guardrails to `.github/instructions/` and `.claude/rules/`, and compiles the guardrails plus portal-local `.apm/instructions/` into `AGENTS.md` and `.github/copilot-instructions.md`. The pipeline's instruction discovery reads those compiled and native files; skills are on-demand workflows and are not part of the acknowledgement contract.
