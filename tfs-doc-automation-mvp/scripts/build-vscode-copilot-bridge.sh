...
# Read from the manifest rather than hardcoded: the artifact name carries the version,
# and a bump that renamed only some of the three places that spell it out would leave
# the installers looking for a file the build no longer writes.
BRIDGE_VERSION="$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["version"])' "$BRIDGE_DIR/package.json")"
OUTPUT_PATH="$BRIDGE_DIR/content-ai-pipeline-bridge-$BRIDGE_VERSION.vsix"
